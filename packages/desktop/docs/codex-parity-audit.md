...
| SSH remote devboxes | Settings → Connections persists/test/connects OpenSSH targets and starts remote `pi --mode rpc` with a streamed approval/plan/goal bridge; the remote host owns the agent, shell, credentials, sessions, workspace, and core Git flow (status, changes, branches, commit, push) | 🟡 core bridge; remote Pi must already be installed, and per-file Git review/PRs, artifact preview, remote worktrees, project-extension trust, and WSL-specific UX remain outstanding |
...
1. Extend the SSH bridge with remote per-file Git review/PRs, artifact preview, worktrees, trust-aware project extensions, Pi installation, and WSL-specific UX.
