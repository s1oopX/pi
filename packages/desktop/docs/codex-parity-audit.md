...
| Installable plugin bundles | Settings → Resources installs, updates, lists, and removes npm/Git/local packages at user or project scope, then hot-reloads bundled extensions, skills, prompts, themes, and Codex command hooks with diagnostics. Pi manifests remain authoritative when present; otherwise `.codex-plugin/plugin.json` loads declared `skills`, maps declared `commands` to prompt templates, runs `SessionStart`, `UserPromptSubmit`, and successful `PostToolUse` hooks, and loads manifest `mcpServers` or root `.mcp.json` HTTP servers. MCP connections expose `mcp__<server>__connect`, register remote tools after connection, complete DCR/PKCE browser OAuth, persist credentials, and restore valid authenticated sessions. Install/update is explicitly confirmed because package code has full system access. | 🟡 runtime bridge; Codex apps, stdio MCP servers, other hook events, and marketplace metadata still need matching runtimes |
...
| Automations / scheduled tasks | Persistent RRULE cron tasks and bound-conversation heartbeats with structured Repeat / On / At controls for hourly, daily, weekday, and weekly schedules, plus advanced RRULE fallback, per-automation model/reasoning, local or dedicated-worktree destinations, notifications, run triage, and reopenable sessions. Create with Pi starts a fresh guided scheduling conversation; history supports Mark all as read and confirmed Archive all actions. Pi package prompts provide the Codex plugin-template equivalent: From Pi packages suggestions populate name, prompt, and schedule, persist a stable package/template link through customization, identify the source package, and restore current package defaults after confirmation. Heartbeats bind main-process-verified session metadata, reuse an existing owning backend when available, and lock the target session while running; worktree cleanup preserves local changes. | ✅ functional parity through Pi packages |

## Loop backlog (ordered)

1. Add Codex app connector identifiers and marketplace metadata for discovery and installation; add stdio MCP only when a verified plugin requires it.
2. Revisit inline legacy `.xls` preview only if the system-app fallback proves insufficient.
