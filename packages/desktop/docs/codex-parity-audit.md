...
| SSH remote devboxes | Settings → Connections persists/test/connects OpenSSH targets and starts remote `pi --mode rpc` with a streamed approval/plan/goal bridge; the remote host owns the agent, shell, credentials, sessions, and workspace | 🟡 core bridge; remote Pi must already be installed, and local Git/artifact panels, remote worktrees, project-extension trust, and WSL-specific UX remain outstanding |
...
1. Extend the SSH bridge with remote Git/artifact panels, worktrees, trust-aware project extensions, Pi installation, and WSL-specific UX.
