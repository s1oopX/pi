...
| SSH remote devboxes | Settings → Connections persists/test/connects OpenSSH targets and starts remote `pi --mode rpc` with a streamed approval/plan/goal bridge; the remote host owns the agent, shell, credentials, sessions, workspace, and the full Git panel, including per-file/hunk review, recoverable discard, and remote `gh` PR workflows | 🟡 advanced bridge; remote Pi must already be installed, and artifact preview, remote worktrees, project-extension trust, and WSL-specific UX remain outstanding |
...
1. Extend the SSH bridge with artifact preview, worktrees, trust-aware project extensions, Pi installation, and WSL-specific UX.
