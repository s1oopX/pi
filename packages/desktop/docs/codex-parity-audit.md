...
| SSH remote devboxes | Settings → Connections persists/test/connects OpenSSH targets and streams the catalog-free Studio Node RPC backend to the host; the remote host owns the agent, shell, credentials, sessions, workspace, full Git panel, and isolated parallel worktrees while retaining the desktop trust/approval/plan/goal contract. Artifact preview/open/reveal streams only the selected file through a descriptor-bound containment check into a bounded private local cache. Settings → Agent lists retained managed worktrees from the active connection, reports dirty state, and explicitly removes them while preserving task branches. An explicit Install / Repair action bootstraps a checksum-pinned private Node.js runtime and the exact matching Pi package on Linux x64/arm64 without sudo, providing the bundle's dependencies and assets. Project extensions and related resources are discovered remotely but remain disabled until the user stores a trust decision on that host; trusting or revoking hot-reloads them in place | 🟡 advanced bridge; WSL-specific UX remains outstanding |
...
1. Add WSL-specific workspace discovery and connection UX.
